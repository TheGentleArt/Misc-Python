...
fig, axs = plt.subplots(4, 1)  # Create a figure with (rows,cols)
# Create the first plot (the original data in time domain)
plt.sca(axs[0])  # First location of subplots
plt.plot(t, f, color='c', linewidth=1.5, label='Original data')
...
plt.sca(axs[1])  # Second location of the subplots
plt.plot(freq, PSD, color='c', linewidth=2, label='PSD')
plt.plot([freq.iloc[0], freq.iloc[-1]], [PSDcutoff, PSDcutoff], color='r', linewidth=2, label='PSDcutoff filter')
...
# =============================================================================
plt.sca(axs[3])
plt.specgram(ffilt, Fs=int(1/dt))
